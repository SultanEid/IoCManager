...
public sealed class LegacySnortQuarantineSessionManager
{
    ...
    private readonly ILogger<LegacySnortQuarantineSessionManager> _logger;

    public LegacySnortQuarantineSessionManager(
        IServiceScopeFactory scopeFactory,
        IOptionsMonitor<ScanExecutionOptions> scanExecutionOptions,
        ILogger<LegacySnortQuarantineSessionManager> logger)
    {
    ...
    internal Task StartAsync(LegacyPipelineSnortQuarantineSessionDefinition definition, CancellationToken cancellationToken)
    {
        if (_activeSessions.ContainsKey(definition.JobId))
        {
            throw new InvalidOperationException($"Snort quarantine job '{definition.JobId}' is already running.");
        }
        ...
        var completionTask = Task.Run(async () =>
        {
            try
            {
                ...
                await service.FinalizeSnortQuarantineJobAsync(definition.JobId, captures, activeSession?.StopRequested == true, CancellationToken.None);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Snort quarantine session for job {JobId} failed.", definition.JobId);
    ...
    public async Task StopAsync(int jobId, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("No active Snort Quarantine session was found for this job.");
        }
    ...
    private async Task<IReadOnlyList<LegacyPipelineSnortQuarantineTargetCapture>> RunAllTargetsAsync(
        LegacyPipelineSnortQuarantineSessionDefinition definition,
        CancellationToken cancellationToken)
    {
        if (definition.Targets.Count > 1)
        {
            ...
            return definition.Targets
                .Select(target => new LegacyPipelineSnortQuarantineTargetCapture(
                    target.TargetId,
    ...
    private async Task<LegacyPipelineSnortQuarantineTargetCapture> RunTargetsAsync(
        LegacyPipelineSnortQuarantineSessionDefinition definition,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!process.Start())
            {
                throw new InvalidOperationException($"Failed to start Snort Quarantine watcher for target set '{string.Join(",", targets.Select(item => item.TargetId))}'.");
            }
            ...
            var failureSummary = !cancellationToken.IsCancellationRequested && process.ExitCode != 0
                ? string.IsNullOrWhiteSpace(standardError)
                    ? $"Snort Quarantine exited with code {process.ExitCode}."
                    : standardError.Trim()
                : null;

            return new LegacyPipelineSnortQuarantineTargetCapture(
    ...
    private async Task MarkJobFailedAsync(int jobId, string summary)
    {
        ...
        job.Summary = string.IsNullOrWhiteSpace(summary)
            ? "Snort quarantine session failed."
