...
public sealed class AiDecisionRequestConfiguration : IEntityTypeConfiguration<AiDecisionRequest>
{
    public void Configure(EntityTypeBuilder<AiDecisionRequest> builder)
    {
        builder.ToTable("ai_decision_requests");
...
public sealed class AiDecisionJobConfiguration : IEntityTypeConfiguration<AiDecisionJob>
{
    public void Configure(EntityTypeBuilder<AiDecisionJob> builder)
    {
        builder.ToTable("ai_decision_jobs");
        builder.HasKey(x => x.Id);
        builder.Property(x => x.Status).HasConversion<string>().HasMaxLength(64).IsRequired();
        ...
        builder.HasOne<AiDecisionRequest>()
            .WithMany()
            .HasForeignKey(x => x.DecisionRequestId)
            .OnDelete(DeleteBehavior.Cascade);

        builder.HasIndex(x => new { x.DecisionRequestId, x.AttemptNumber }).IsUnique();
        builder.HasIndex(x => new { x.Status, x.QueuedAtUtc });
        builder.HasIndex(x => x.NextAttemptAtUtc);
    }
}

public sealed class AiDecisionResultConfiguration : IEntityTypeConfiguration<AiDecisionResult>
{
    public void Configure(EntityTypeBuilder<AiDecisionResult> builder)
    {
        builder.ToTable("ai_decision_results", table =>
        {
            table.HasCheckConstraint("ck_ai_decision_results_confidence", "[Confidence] >= 0 AND [Confidence] <= 1");
            table.HasCheckConstraint("ck_ai_decision_results_false_positive_risk", "[FalsePositiveRisk] >= 0 AND [FalsePositiveRisk] <= 1");
        });
        builder.HasKey(x => x.Id);
        ...
        builder.HasIndex(x => x.DecisionRequestId).IsUnique();
        builder.HasIndex(x => x.ScoredAtUtc);
    }
}

public sealed class AiDecisionExplanationConfiguration : IEntityTypeConfiguration<AiDecisionExplanation>
{
    public void Configure(EntityTypeBuilder<AiDecisionExplanation> builder)
    {
        builder.ToTable("ai_decision_explanations");
        builder.HasKey(x => x.Id);
...
public sealed class AiDecisionOverrideConfiguration : IEntityTypeConfiguration<AiDecisionOverride>
{
    public void Configure(EntityTypeBuilder<AiDecisionOverride> builder)
    {
        builder.ToTable("ai_decision_overrides");
        builder.HasKey(x => x.Id);
        ...
        builder.HasIndex(x => new { x.DecisionRequestId, x.SubmittedAtUtc });
    }
}

public sealed class AiDecisionSimilarDetectionConfiguration : IEntityTypeConfiguration<AiDecisionSimilarDetection>
{
    public void Configure(EntityTypeBuilder<AiDecisionSimilarDetection> builder)
    {
        builder.ToTable("ai_decision_similar_detections", table =>
        {
            table.HasCheckConstraint("ck_ai_decision_similar_detections_confidence", "[Confidence] >= 0 AND [Confidence] <= 1");
            table.HasCheckConstraint("ck_ai_decision_similar_detections_similarity", "[SimilarityScore] >= 0 AND [SimilarityScore] <= 1");
        });
        builder.HasKey(x => x.Id);
        builder.Property(x => x.DetectionId).HasMaxLength(128).IsRequired();
        ...
        builder.HasIndex(x => new { x.DecisionRequestId, x.Rank });
        builder.HasIndex(x => new { x.DecisionRequestId, x.ObservedAtUtc });
    }
}

public sealed class AiDecisionEvidenceSourceConfiguration : IEntityTypeConfiguration<AiDecisionEvidenceSource>
{
    public void Configure(EntityTypeBuilder<AiDecisionEvidenceSource> builder)
    {
        builder.ToTable("ai_decision_evidence_sources", table =>
        {
            table.HasCheckConstraint("ck_ai_decision_evidence_sources_confidence", "[Confidence] IS NULL OR ([Confidence] >= 0 AND [Confidence] <= 1)");
        });
        builder.HasKey(x => x.Id);
