...
public sealed class AiDecisionService : IAiDecisionService
{
    ...
    private readonly IAiDecisionRepository _repository;
    private readonly IAiDecisionOrchestrator _orchestrator;

    public AiDecisionService(
        IAiDecisionRepository repository,
    ...
    public async Task<SubmitDecisionAcceptedDto> SubmitAsync(SubmitDecisionRequestDto request, CancellationToken cancellationToken)
    {
        var nowUtc = DateTimeOffset.UtcNow;
        Guid? detectionRecordId = Guid.TryParse(request.DetectionId, out var parsedDetectionId) ? parsedDetectionId : null;
        var row = AiDecisionRequest.Queue(
        ...
        return new SubmitDecisionAcceptedDto(
            DecisionId: row.Id,
    ...
    public async Task<IocLatestDecisionDto?> GetLatestResultByIocAsync(Guid iocId, CancellationToken cancellationToken)
    {
        ...
        var result = await GetResultAsync(reference.DecisionRequestId, cancellationToken);
        if (result is null)
        {
            return null;
        }
    ...
    public async Task<DecisionResultDto?> GetLatestResultByDetectionAsync(Guid detectionId, CancellationToken cancellationToken)
    {
    ...
    public async Task<DecisionResultDto?> GetResultAsync(Guid decisionId, CancellationToken cancellationToken)
    {
        var request = await _repository.GetRequestAsync(decisionId, asTracking: false, cancellationToken);
        if (request is null)
        {
            return null;
        }

        var result = await _repository.GetResultAsync(decisionId, asTracking: false, cancellationToken);
        var explanation = await _repository.GetExplanationAsync(decisionId, asTracking: false, cancellationToken);
        var actionPlan = await _repository.GetActionPlanAsync(decisionId, asTracking: false, cancellationToken);
        var similarSlice = await _repository.ListSimilarDetectionsAsync(decisionId, skip: 0, take: 1, cancellationToken);
        var evidenceSlice = await _repository.ListEvidenceSourcesAsync(decisionId, skip: 0, take: 1, cancellationToken);
    ...
    public async Task<ExplanationDetailDto?> GetExplanationAsync(Guid decisionId, CancellationToken cancellationToken)
    {
        var request = await _repository.GetRequestAsync(decisionId, asTracking: false, cancellationToken);
        if (request is null)
        {
    ...
    public async Task<RecommendedActionPlanDto?> GetActionPlanAsync(Guid decisionId, CancellationToken cancellationToken)
    {
        var request = await _repository.GetRequestAsync(decisionId, asTracking: false, cancellationToken);
        if (request is null)
        {
        ...
        var plan = await _repository.GetActionPlanAsync(decisionId, asTracking: false, cancellationToken);
        if (plan is null)
        {
    ...
    {
        var row = await _repository.GetRequestAsync(decisionId, asTracking: true, cancellationToken);
        if (row is null)
        {
        ...
        var item = AiDecisionOverride.Create(
    ...
    public async Task<SimilarDetectionsResponseDto?> GetSimilarDetectionsAsync(
        ...
        CancellationToken cancellationToken)
    {
        var row = await _repository.GetRequestAsync(decisionId, asTracking: false, cancellationToken);
        if (row is null)
        {
            return null;
        }
        ...
        var slice = await _repository.ListSimilarDetectionsAsync(decisionId, offset, boundedLimit, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var boundedLimit = Math.Clamp(limit, 1, 100);
        var offset = ParseCursor(cursor);
        var slice = await _repository.ListEvidenceSourcesAsync(decisionId, offset, boundedLimit, cancellationToken);
        var nextCursor = slice.HasMore ? EncodeCursor(offset + boundedLimit) : null;
    ...
    private static DecisionLinksDto BuildLinks(Guid decisionId)
    {
        var root = $"/api/v2/ai/decisions/{decisionId:D}";
        return new DecisionLinksDto(
            Result: root,
    ...
    private static DecisionDecisionDto ToDecisionDto(AiDecisionResult result)
    {
        ...
        return new DecisionDecisionDto(
    ...
    private static SimilarDetectionDto ToSimilarDetectionDto(AiDecisionSimilarDetection item)
    {
    ...
    private static EvidenceSourceDto ToEvidenceDto(AiDecisionEvidenceSource item)
    {
