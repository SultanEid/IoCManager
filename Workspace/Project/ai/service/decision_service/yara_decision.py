...
def _suggested_next_checks(
    *,
    ...
    checks: list[str] = []
    if lexical_only_gate_triggered:
        checks.append("Collect non-lexical corroboration (behavior telemetry or trusted enrichment) before decision.")
...
def _explanation_lines(
    *,
    ...
    lines = [
        (
            "YARA deterministic decision produced "
    ...
    if lexical_only_gate_triggered:
        lines.append("Lexical-only evidence was detected; decision abstained until non-lexical corroboration is available.")
    else:
        lines.append(
