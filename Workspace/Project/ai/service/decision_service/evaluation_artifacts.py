    ...
    *,
    ...
    with calibration_csv_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for row in calibration_rows:
            writer.writerow(_normalize_calibration_row(row))
...
def _normalize_calibration_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "index": row.get("index"),
        "lower_bound": row.get("lower_bound", row.get("lowerBound")),
        "upper_bound": row.get("upper_bound", row.get("upperBound")),
        "sample_size": row.get("sample_size", row.get("sampleSize")),
        "average_confidence": row.get("average_confidence", row.get("averageConfidence")),
        "empirical_positive_rate": row.get("empirical_positive_rate", row.get("empiricalPositiveRate")),
    }
