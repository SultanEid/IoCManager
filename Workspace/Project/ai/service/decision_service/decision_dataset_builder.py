...
TASK_YARA = "yara_package_decision"
TASK_SIGMA = "sigma_package_decision"
TASK_SNORT = "snort_package_decision"
...
def build_decision_dataset(config: DatasetBuildConfig) -> dict[str, Any]:
...
def register_dataset_build(
    *,
    ...
    entry = DatasetRegistryEntry(
        ...
        notes=notes or "Persisted by build_decision_dataset.",
    )
...
def _normalize_row(raw: dict[str, Any], dataset_version: str) -> dict[str, Any]:
    ...
    _apply_deterministic_yara_decision(
    ...
    _apply_deterministic_snort_decision(
        rule_family=rule_family,
        package_payload=package_payload,
        target_payload=target_payload,
        evidence_used=evidence_used,
    ...
    if contradictory_evidence:
        target_decision = target_payload.get("decision")
        if isinstance(target_decision, dict):
            target_decision["contradictory_evidence"] = contradictory_evidence
    ...
    if "action_plan" not in target_payload and "decision" in target_payload:
        derived = _derive_action_plan_from_decision(target_payload["decision"], evidence_used)
...
def _extract_target_payload(payload: dict[str, Any], scenario_label: str | None) -> dict[str, Any]:
    ...
    explicit_decision = payload.get("decision_result") or payload.get("decision")
    if isinstance(explicit_decision, dict):
        target["decision"] = explicit_decision
    ...
    if "decision" not in target and scenario_label:
...
def _extract_evidence(
    ...
    if not isinstance(evidence_used, list) and isinstance(target_payload.get("decision"), dict):
        maybe_used = target_payload["decision"].get("evidence_used")
    ...
    if not isinstance(evidence_missing, list) and isinstance(target_payload.get("decision"), dict):
        maybe_missing = target_payload["decision"].get("evidence_missing")
        if isinstance(maybe_missing, list):
            evidence_missing = maybe_missing
    if not isinstance(evidence_missing, list):
        verdict = (
            target_payload.get("decision", {}).get("verdict")
            if isinstance(target_payload.get("decision"), dict)
            else None
        )
...
def _fuse_row_evidence(
    *,
    ...
    if isinstance(target_payload.get("decision"), dict):
        ...
        target_payload["decision"]["evidence_fusion"] = {
...
def _apply_deterministic_yara_decision(
    *,
    rule_family: str,
    package_payload: dict[str, Any],
    target_payload: dict[str, Any],
    ...
    if not isinstance(decision, dict):
        decision = {}
        target_payload["decision"] = decision

    _backfill_decision_from_yara(
        decision=decision,
        generated=generated,
        evidence_used=evidence_used,
        evidence_missing=evidence_missing,
    ...
    if "evidence_fusion" not in decision or not isinstance(decision.get("evidence_fusion"), dict):
        decision["evidence_fusion"] = {
...
def _backfill_decision_from_yara(
    *,
    decision: dict[str, Any],
    generated: YaraDecisionResult,
    evidence_used: list[dict[str, Any]],
    evidence_missing: list[dict[str, Any]],
    contradictory_evidence: list[dict[str, Any]],
) -> None:
    existing_verdict = decision.get("verdict")
    verdict_present = isinstance(existing_verdict, str) and bool(existing_verdict.strip())
    if not verdict_present:
        decision["verdict"] = generated.verdict

    if "confidence" not in decision or decision.get("confidence") is None:
        decision["confidence"] = generated.confidence
    if "false_positive_risk" not in decision or decision.get("false_positive_risk") is None:
        decision["false_positive_risk"] = generated.false_positive_risk
    if "explanation" not in decision or not str(decision.get("explanation", "")).strip():
        decision["explanation"] = generated.explanation
    if "suggested_next_checks" not in decision or not isinstance(decision.get("suggested_next_checks"), list):
        decision["suggested_next_checks"] = list(generated.suggested_next_checks)
    if "evidence_used" not in decision or not isinstance(decision.get("evidence_used"), list):
        decision["evidence_used"] = _merge_object_lists([], evidence_used)
    if "evidence_missing" not in decision or not isinstance(decision.get("evidence_missing"), list):
        decision["evidence_missing"] = _merge_object_lists([], evidence_missing)
    if "contradictory_evidence" not in decision or not isinstance(decision.get("contradictory_evidence"), list):
        decision["contradictory_evidence"] = _merge_object_lists([], contradictory_evidence)
    if (
        (decision.get("verdict") == "insufficient_evidence" or generated.verdict == "insufficient_evidence")
        and ("abstain_reason" not in decision or not str(decision.get("abstain_reason", "")).strip())
    ):
        decision["abstain_reason"] = generated.abstain_reason or "insufficient_correlated_evidence"

    if "bucket_scores" not in decision or not isinstance(decision.get("bucket_scores"), dict):
        decision["bucket_scores"] = {
    ...
    if "interpretable_features" not in decision or not isinstance(decision.get("interpretable_features"), dict):
        decision["interpretable_features"] = dict(generated.interpretable_features)
    ...
    if verdict_present and isinstance(existing_verdict, str):
        ...
        if existing_value in CANONICAL_YARA_VERDICTS:
            decision["verdict"] = existing_value


def _apply_deterministic_snort_decision(
    *,
    rule_family: str,
    package_payload: dict[str, Any],
    target_payload: dict[str, Any],
...
def _backfill_decision_from_snort(
    *,
    decision: dict[str, Any],
    generated: SnortDecisionResult,
    evidence_used: list[dict[str, Any]],
    evidence_missing: list[dict[str, Any]],
    contradictory_evidence: list[dict[str, Any]],
) -> None:
...
def _extract_provenance(
    ...
    source_item = {
        ...
        "retrieved_by": "decision_dataset_builder_v1",
...
def _derive_action_plan_from_decision(
    decision: dict[str, Any],
    evidence_used: list[dict[str, Any]],
) -> dict[str, Any] | None:
...
def _derive_eligible_tasks(rule_family: str, target_payload: dict[str, Any]) -> list[str]:
    ...
    if isinstance(target_payload.get("action_plan"), dict) or isinstance(target_payload.get("decision"), dict):
...
def _derive_missing_fields(
    ...
    if any(task in eligible_tasks for task in (TASK_YARA, TASK_SIGMA, TASK_SNORT)):
        if not isinstance(decision, dict) or not str(decision.get("verdict", "")).strip():
            missing.append("target_payload.decision.verdict")
...
def _is_split_eligible(task: str, row: dict[str, Any]) -> bool:
    ...
    if task in {TASK_YARA, TASK_SIGMA, TASK_SNORT}:
        ...
        return isinstance(decision, dict) and bool(str(decision.get("verdict", "")).strip())
...
def _write_snapshot_exports(*, dataset_root: Path, normalized_rows: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for row in normalized_rows:
        ...
        verdict = _coerce_dict(_coerce_dict(row.get("target_payload")).get("decision")).get("verdict")
...
def _derive_severity_score(row: dict[str, Any]) -> float:
    decision = _coerce_dict(_coerce_dict(row.get("target_payload")).get("decision"))
    confidence = _float_or_none(decision.get("confidence")) or 0.0
...
def _render_training_row_report(rows: list[dict[str, Any]]) -> str:
    header = [
        "# Training Row Format",
        "",
        "Canonical JSONL rows produced by `build_decision_dataset.py`.",
        "",
        "## Canonical Fields",
    ]
...
def _empty_example_row() -> dict[str, Any]:
    return {
        ...
        "is_partial": True,
        "missing_fields": ["target_payload.decision.verdict"],
