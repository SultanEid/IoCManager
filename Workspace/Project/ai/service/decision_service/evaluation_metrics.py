...
def compute_decision_metrics(
    rows: Iterable[DecisionEvaluationRow],
    *,
    threshold: float,
    top_k: int = 10,
...
) -> list[DecisionSliceSummary]:
    items = list(rows)
    if not items:
        return []

    resolved_reference_time = reference_time or max(item.event_time for item in items)
    grouped: dict[tuple[str, str], list[DecisionEvaluationRow]] = {}
    for field in slice_fields:
        for item in items:
            key = _decision_slice_value(field, item, resolved_reference_time)
            grouped.setdefault((field, key), []).append(item)

    output: list[DecisionSliceSummary] = []
...
def _decision_slice_value(field: str, row: DecisionEvaluationRow, reference_time: datetime) -> str:
