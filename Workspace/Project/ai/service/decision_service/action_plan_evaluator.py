...
def build_action_plan_evaluation_row(row: dict[str, Any]) -> ActionPlanEvaluationRow | None:
    ...
    decision = target_payload.get("decision") if isinstance(target_payload.get("decision"), dict) else {}
    ...
    verdict = str(decision.get("verdict", "insufficient_evidence")).strip().lower() or "insufficient_evidence"
    confidence = _clip01(float(decision.get("confidence", _default_confidence(verdict))))
    false_positive_risk = _clip01(float(decision.get("false_positive_risk", _default_false_positive_risk(verdict))))
    evidence_fusion = _build_evidence_fusion(decision)
    evidence_summary = _coerce_string_list(
        [
            decision.get("explanation"),
            *(item.get("summary") for item in _coerce_dict_list(decision.get("evidence_used"))),
        ]
    )
    ...
    return ActionPlanEvaluationRow(
        ...
        evidence_used_count=len(_coerce_dict_list(decision.get("evidence_used"))),
        evidence_missing_count=len(_coerce_dict_list(decision.get("evidence_missing"))),
    )
...
def _build_evidence_fusion(decision: dict[str, Any]) -> EvidenceFusionExplanationResponse:
    positive = [
        EvidenceFusionEvidenceItemResponse(
        ...
        for item in _coerce_dict_list(decision.get("evidence_used"))
    ]
    contradictory = [
        EvidenceFusionEvidenceItemResponse(
            channel="dataset_label",
        ...
        for item in _coerce_dict_list(decision.get("contradictory_evidence"))
    ]
    missing = [
        EvidenceFusionMissingItemResponse(
        ...
        for index, item in enumerate(_coerce_dict_list(decision.get("evidence_missing")), start=1)
    ]
    coverage = {
