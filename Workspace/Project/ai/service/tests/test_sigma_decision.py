...
from decision_service.sigma_decision import adjudicate_sigma
...
def test_sigma_decision_is_deterministic_for_identical_input() -> None:
...
def test_sigma_decision_extracts_interpretable_features_for_required_inputs() -> None:
...
def test_sigma_decision_scores_evidence_buckets_independently() -> None:
    result = adjudicate_sigma(detection_package=_malicious_package())
...
def test_sigma_decision_hard_abstains_when_only_lexical_evidence_exists() -> None:
...
def test_sigma_decision_history_signals_change_bucket_outputs() -> None:
...
def test_sigma_decision_classifies_into_runtime_verdict_taxonomy(
    builder,
...
def test_sigma_decision_distinguishes_benign_admin_from_suspicious_admin_mismatch() -> None:
...
def test_sigma_decision_produces_explanation_and_suggested_checks() -> None:
    result = adjudicate_sigma(detection_package=_insufficient_evidence_package())
    assert "SIGMA deterministic decision produced verdict=" in result.explanation
