...
from cti_service.yara_adjudication import adjudicate_yara
...
def test_yara_adjudication_extracts_interpretable_features_for_all_required_inputs() -> None:
...
def test_yara_adjudication_scores_evidence_buckets_independently() -> None:
    result = adjudicate_yara(detection_package=_malicious_package())
...
def test_yara_adjudication_hard_abstains_when_only_lexical_evidence_exists() -> None:
...
def test_yara_adjudication_history_signals_change_bucket_outputs() -> None:
...
def test_yara_adjudication_classifies_into_runtime_verdict_taxonomy(
    builder,
...
def test_yara_adjudication_produces_explanation_and_suggested_checks() -> None:
    result = adjudicate_yara(detection_package=_insufficient_evidence_package())
    assert "YARA deterministic adjudication produced verdict=" in result.explanation
