...
def test_yara_rows_without_decision_are_backfilled_by_deterministic_pipeline() -> None:
    normalized = _normalize_row(
        raw={
            "raw": {
                "rule_family": "yara",
    ...
    decision = normalized["target_payload"]["decision"]
    ...
    assert "bucket_scores" in decision
    assert "interpretable_features" in decision
    assert isinstance(decision.get("suggested_next_checks"), list)
...
def test_yara_backfill_is_deterministic_for_identical_rows() -> None:
    ...
    assert first["target_payload"]["decision"] == second["target_payload"]["decision"]
...
def test_yara_backfill_preserves_explicit_verdict_and_confidence() -> None:
    ...
    assert decision["false_positive_risk"] == 0.03
    assert isinstance(decision.get("suggested_next_checks"), list)
    assert isinstance(decision.get("bucket_scores"), dict)


def test_snort_rows_without_decision_are_backfilled_by_deterministic_pipeline() -> None:
    normalized = _normalize_row(
        raw={
            "raw": {
                "rule_family": "snort",
...
def test_snort_backfill_preserves_explicit_verdict_and_confidence() -> None:
    ...
    assert decision["false_positive_risk"] == 0.07
    assert isinstance(decision.get("suggested_next_checks"), list)
    assert isinstance(decision.get("bucket_scores"), dict)
...
def test_row_evidence_fusion_deduplicates_overlapping_payload_and_adapter_evidence() -> None:
    ...
    dedup = normalized["target_payload"]["decision"]["evidence_fusion"]["deduplication"]
...
def test_row_evidence_fusion_surfaces_contradictory_analyst_history() -> None:
    ...
    assert decision["evidence_fusion"]["contradictory_count"] > 0
...
def test_row_evidence_fusion_generates_missing_evidence_for_sparse_inputs() -> None:
    payload = {
        ...
        "decision_result": {"verdict": "insufficient_evidence"},
    }
    normalized = _normalize_row(
        raw={
            "raw": payload,
...
def test_end_to_end_build_writes_canonical_task_split_and_report_files(tmp_path: Path) -> None:
    ...
    assert all(isinstance(row["target_payload"].get("decision"), dict) for row in imported_rows)
    assert all(str(row["target_payload"]["decision"].get("verdict", "")).strip() for row in imported_rows)
    ...
    yara_split_rows = []
    for split_name in ("train", "validation", "test"):
        yara_split_rows.extend(_read_jsonl(Path(split_manifest["files"]["splits"]["yara_package_decision"][split_name])))
...
def test_manifest_dispatch_supports_new_sigma_context_parsers(tmp_path: Path) -> None:
    ...
    assert analyst_row["target_payload"]["decision"]["verdict"] == "likely_malicious"
