...
from .sigma_adjudication import SigmaAdjudicationResult, adjudicate_sigma
...
def _build_sigma_grounded_decision(
    *,
    ...
    reasons = list(sigma_adjudication.explanation_lines)[:4]
    ...
    next_best_evidence = list(sigma_adjudication.suggested_next_checks)
    if not next_best_evidence:
        next_best_evidence = _next_best_evidence(score=score, verdict=verdict)
    abstain_reason = None
    if verdict == "insufficient_evidence":
        abstain_reason = sigma_adjudication.abstain_reason or "insufficient_correlated_evidence"
    provenance = _provenance(
        score=score,
        request=request,
        evidence_fusion=evidence_fusion,
        sigma_adjudication=sigma_adjudication,
    ...
    return _finalize_grounded_decision(
        ...
        lexical_only_gate_triggered=sigma_adjudication.lexical_only_gate_triggered,
        non_lexical_corroborated=sigma_adjudication.non_lexical_corroborated,
        contradiction_score=sigma_adjudication.bucket_scores.contradictory_score,
        missing_score=sigma_adjudication.bucket_scores.missing_score,
        fusion_failure_reason=fusion_failure_reason,
    )
...
def _build_snort_grounded_decision(
    *,
    ...
    detection_package = request.detection_package if isinstance(request.detection_package, dict) else {}
    snort_adjudication = adjudicate_snort(
        detection_package=detection_package,
        evidence_fusion=evidence_fusion,
        historical_learning_features=_historical_learning_features(historical_learning),
    )
    verdict = snort_adjudication.verdict
    action = _select_action(
        verdict=verdict,
        uncertainty=score.uncertainty_score,
        blast_radius=score.blast_radius_score,
    ...
    reasons = list(snort_adjudication.explanation_lines)[:4]
    if evidence_fusion is not None and len(reasons) < 4:
        reasons.append(
            "evidence_fusion="
            f"positive:{len(evidence_fusion.positive_evidence)}, "
    ...
    next_best_evidence = list(snort_adjudication.suggested_next_checks)
    if not next_best_evidence:
        next_best_evidence = _next_best_evidence(score=score, verdict=verdict)
    abstain_reason = None
    if verdict == "insufficient_evidence":
        abstain_reason = snort_adjudication.abstain_reason or "insufficient_correlated_evidence"
    provenance = _provenance(
        score=score,
        request=request,
        evidence_fusion=evidence_fusion,
        snort_adjudication=snort_adjudication,
        historical_learning=historical_learning,
    )
    return _finalize_grounded_decision(
        score=score,
        ...
        lexical_only_gate_triggered=snort_adjudication.lexical_only_gate_triggered,
        non_lexical_corroborated=snort_adjudication.non_lexical_corroborated,
        contradiction_score=snort_adjudication.bucket_scores.contradictory_score,
        missing_score=snort_adjudication.bucket_scores.missing_score,
        fusion_failure_reason=fusion_failure_reason,
    )
...
def _build_yara_grounded_decision(
    *,
    ...
    detection_package = request.detection_package if isinstance(request.detection_package, dict) else {}
    yara_adjudication = adjudicate_yara(
        detection_package=detection_package,
        evidence_fusion=evidence_fusion,
        historical_learning_features=_historical_learning_features(historical_learning),
    )
    verdict = yara_adjudication.verdict
    action = _select_action(
        verdict=verdict,
        uncertainty=score.uncertainty_score,
        blast_radius=score.blast_radius_score,
    ...
    reasons = list(yara_adjudication.explanation_lines)[:4]
    if evidence_fusion is not None and len(reasons) < 4:
        reasons.append(
            "evidence_fusion="
            f"positive:{len(evidence_fusion.positive_evidence)}, "
    ...
    next_best_evidence = list(yara_adjudication.suggested_next_checks)
    if not next_best_evidence:
        next_best_evidence = _next_best_evidence(score=score, verdict=verdict)
    abstain_reason = None
    if verdict == "insufficient_evidence":
        abstain_reason = yara_adjudication.abstain_reason or "insufficient_correlated_evidence"
    provenance = _provenance(
        score=score,
        request=request,
        evidence_fusion=evidence_fusion,
        yara_adjudication=yara_adjudication,
        historical_learning=historical_learning,
    )
    return _finalize_grounded_decision(
        score=score,
        ...
        lexical_only_gate_triggered=yara_adjudication.lexical_only_gate_triggered,
        non_lexical_corroborated=yara_adjudication.non_lexical_corroborated,
        contradiction_score=yara_adjudication.bucket_scores.contradictory_score,
        missing_score=yara_adjudication.bucket_scores.missing_score,
        fusion_failure_reason=fusion_failure_reason,
    )
...
def _safety_reason_lines(*, safety_inputs: _SafetyInputs, safety_outcome: _SafetyOutcome) -> list[str]:
    ...
    if safety_inputs.enrichment_status != "available":
        reasons.append(
            ...
            + "; adjudication continued with missing-evidence pressure."
        )
    return reasons
...
def _log_grounded_decision_safety(
    *,
    ...
    if verdict == "insufficient_evidence":
        _log_safety_event(logging.WARNING, "adjudication_abstained", **log_fields)
    if safety_inputs.partial_evidence:
        _log_safety_event(logging.INFO, "adjudication_partial_evidence", **log_fields)
    if safety_inputs.enrichment_status != "available":
        _log_safety_event(logging.INFO, "adjudication_enrichment_degraded", **log_fields)
...
def _provenance(
    ...
    evidence_fusion: EvidenceFusionExplanationResponse | None,
    historical_learning: HistoricalLearningContextResponse | None = None,
    yara_adjudication: YaraAdjudicationResult | None = None,
    sigma_adjudication: SigmaAdjudicationResult | None = None,
    snort_adjudication: SnortAdjudicationResult | None = None,
    ...
    if yara_adjudication is not None:
        output.extend(
            [
                DecisionProvenanceItemResponse(
                    source="yara_adjudication",
                    key="positive_score",
                    value=f"{yara_adjudication.bucket_scores.positive_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="yara_adjudication",
                    key="negative_score",
                    value=f"{yara_adjudication.bucket_scores.negative_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="yara_adjudication",
                    key="contradictory_score",
                    value=f"{yara_adjudication.bucket_scores.contradictory_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="yara_adjudication",
                    key="missing_score",
                    value=f"{yara_adjudication.bucket_scores.missing_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="yara_adjudication",
                    key="lexical_only_gate_triggered",
                    value=str(yara_adjudication.lexical_only_gate_triggered),
                ),
            ]
        )
        for key, value in list(yara_adjudication.interpretable_features.items())[:8]:
            output.append(
                DecisionProvenanceItemResponse(
                    source="yara_feature",
                    key=key,
    ...
    if sigma_adjudication is not None:
        output.extend(
            [
                DecisionProvenanceItemResponse(
                    source="sigma_adjudication",
                    key="positive_score",
                    value=f"{sigma_adjudication.bucket_scores.positive_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="sigma_adjudication",
                    key="negative_score",
                    value=f"{sigma_adjudication.bucket_scores.negative_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="sigma_adjudication",
                    key="contradictory_score",
                    value=f"{sigma_adjudication.bucket_scores.contradictory_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="sigma_adjudication",
                    key="missing_score",
                    value=f"{sigma_adjudication.bucket_scores.missing_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="sigma_adjudication",
                    key="lexical_only_gate_triggered",
                    value=str(sigma_adjudication.lexical_only_gate_triggered),
                ),
            ]
        )
        for key, value in list(sigma_adjudication.interpretable_features.items())[:8]:
            output.append(
                DecisionProvenanceItemResponse(
    ...
    if snort_adjudication is not None:
        output.extend(
            [
                DecisionProvenanceItemResponse(
                    source="snort_adjudication",
                    key="positive_score",
                    value=f"{snort_adjudication.bucket_scores.positive_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="snort_adjudication",
                    key="negative_score",
                    value=f"{snort_adjudication.bucket_scores.negative_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="snort_adjudication",
                    key="contradictory_score",
                    value=f"{snort_adjudication.bucket_scores.contradictory_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="snort_adjudication",
                    key="missing_score",
                    value=f"{snort_adjudication.bucket_scores.missing_score:.6f}",
                ),
                DecisionProvenanceItemResponse(
                    source="snort_adjudication",
                    key="lexical_only_gate_triggered",
                    value=str(snort_adjudication.lexical_only_gate_triggered),
                ),
            ]
        )
        for key, value in list(snort_adjudication.interpretable_features.items())[:8]:
            output.append(
                DecisionProvenanceItemResponse(
