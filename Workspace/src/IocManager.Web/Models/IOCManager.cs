...
namespace IoCManager.Mvc.Models
{
    public class IOCManager
    {
        ...
        {
            var plan = new ScanPlan(1, "Default Plan", "Manual");
        ...
        public Report? runScanPlan(ScanPlan plan, Network network)
        {
            ...
            if (plan.AssignedSweeper == null)
                return null;

           
            plan.AssignedSweeper.sweepNetwork(network);

         
            var report = plan.AssignedSweeper.generateSweeperReport();
        ...
        public Report GenerateReports(ScanPlan plan)
        {
            // لازم Report عندك ينشأ بـ (id, title)
            var report = new Report(plan?.PlanID ?? 0, "ScanPlan Report")
            {
                format = "Summary",
