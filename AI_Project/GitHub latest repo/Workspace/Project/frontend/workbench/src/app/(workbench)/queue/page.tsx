...
export default function TriageQueuePage() {
  ...
  if (!isModeConfigured) {
    ...
    return <ClassifiedFailureState failure={failure} fallbackTitle="Queue unavailable" />
  }

  const queueQuery = useWorkbenchQuery(["queue", "rows"], (signal) => buildQueueRows(signal))
  ...
  if (queueQuery.isError || !queueQuery.data) {
    return <ClassifiedFailureState failure={classifyUiError(queueQuery.error)} fallbackTitle="Queue unavailable" />
  }
    ...
    <motion.section className="wb-page" variants={staggerMotion} initial="hidden" animate="visible">
      <motion.header className="wb-page-header" variants={panelMotion}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="wb-kicker">Triage Queue</p>
            <h2 className="mt-1 text-lg font-semibold tracking-tight">Active alerts with real decision lookup status</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Queue rows come from live alerts only. Decision state stays explicitly unavailable when per-alert decision contracts fail.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-lg border border-border/70 bg-surface-2/65 p-3">
            <p className="wb-kicker">Active Queue Rows</p>
          ...
          <div className="rounded-lg border border-amber-300/35 bg-amber-500/10 p-3">
            <p className="wb-kicker text-amber-100">Decision Lookup Gaps</p>
        ...
        {queueQuery.data.totalAlerts > queueQuery.data.rows.length ? (
          <div className="mt-3 rounded-lg border border-border/70 bg-surface-2/55 px-3 py-2 text-xs text-muted-foreground">
